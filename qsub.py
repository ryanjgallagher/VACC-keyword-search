...
while jobs_remaining > batch_size:
    ...
    # if date > datetime.datetime(2013,11,1):
...
/usr/bin/time -v gzip -cd /users/c/d/cdanfort/scratch/twitter/tweet-troll/zipped-raw/{0}/{0}-{1:02d}-{{00,15,30,45}}.gz | python processTweets.py "{0}-{1:02d}"

echo "job script completed"'''.format(date.strftime('%Y-%m-%d'),hour)
