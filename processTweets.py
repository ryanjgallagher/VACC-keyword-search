...
def tweetreader(tweet,outfile):
    ...
    for keyword in keywords:
        if keyword["re"].search(tweet["text"]) is not None:
            ...
            # print(tweet["text"])
            g = codecs.open("raw-tweets/{0}/{1}.json".format(keyword["folder"],outfile),"a","utf8")
            g.write(dumps(tweet))
            g.write("\n")
            g.close()
        # else:
...
keywords = [{"re": re.compile(r"\beach\b",flags=re.IGNORECASE), "folder": "each",},
            {"re": re.compile(r"\bkeyword\b",flags=re.IGNORECASE), "folder": "keyword",},]
